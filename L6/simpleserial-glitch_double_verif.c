...
#endif
{
    ...
    // define C tmp variable for ASM uses
    int tmp1, tmp2;
    // tmp3 and tmp4 are left empty and added to the ASM context, so you can use
    // them in you counter-measures if needed
    int tmp3, tmp4;
    ...
    // 1st version : Duplicated Checks
    __TAB "ldrb %[tmp3], [%[pw], %[cnt]]" __NEW_LINE
    __TAB "ldrb %[tmp4], [%[passwd], %[cnt]]" __NEW_LINE
    __TAB "cmp %[tmp3], %[tmp4]" __NEW_LINE
    __TAB "beq .cL2" __NEW_LINE
